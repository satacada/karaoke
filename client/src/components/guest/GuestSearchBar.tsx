...
export const GuestSearchBar: FC<GuestSearchBarProps> = ({
  query, onQueryChange, onSearch, selectedFilter, onFilterChange, isLoading = false,
}) => {
    ...
    <div className="space-y-2">
      <form onSubmit={handleSubmit} className="relative flex items-center">
        <button
          type="submit"
          disabled={!query.trim()}
          className="absolute left-3 p-1 text-zinc-400 hover:text-emerald-400 disabled:opacity-40 transition-colors cursor-pointer"
        ...
        <div className="absolute right-2.5 flex items-center gap-1">
          {isLoading ? (
            ...
            <button type="button" onClick={() => onQueryChange('')} className="p-1 text-zinc-400 hover:text-white transition-colors" aria-label="Limpiar búsqueda">
              <X className="w-4 h-4" />
            </button>
          ) : null}
          <button
            ...
            className={`p-2 rounded-xl transition-all ${
              isListening ? 'bg-rose-600 text-white animate-pulse shadow-lg shadow-rose-600/40 ring-2 ring-rose-400' : 'bg-zinc-800/80 text-emerald-400 hover:bg-zinc-700 hover:text-white'
            }`}
