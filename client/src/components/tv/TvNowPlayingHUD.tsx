        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm md:text-base text-zinc-400 font-medium truncate">{song.author}</p>
            </div>
          </div>

          <div className="flex items-center gap-2.5">
            {/* Botón interactivo de puntero/toque para pausar o continuar */}
            {onTogglePlayPause && (
              <button
                type="button"
                onClick={onTogglePlayPause}
                className={`flex items-center gap-2 px-4 py-2 rounded-2xl font-bold text-xs uppercase tracking-wider transition-all shadow-lg border cursor-pointer select-none pointer-events-auto active:scale-95 ${
                  isPaused
                    ? 'bg-gradient-to-r from-emerald-600 to-teal-600 border-emerald-400 text-white shadow-emerald-950/50 hover:brightness-110 animate-pulse'
                    : 'bg-gradient-to-r from-pink-600 via-purple-600 to-indigo-600 border-pink-400/50 text-white shadow-purple-950/50 hover:brightness-110'
                }`}
                title={isPaused ? 'Continuar música (Click con puntero o toque)' : 'Pausar música (Click con puntero o toque)'}
              >
                {isPaused ? (
                  <>
