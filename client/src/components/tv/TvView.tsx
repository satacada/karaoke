...
export const TvView: FC<TvViewProps> = ({ roomCode = 'FIESTA' }) => {
  ...
  const handleRemoteCommand = useCallback((cmd: RemoteCommand) => {
    switch (cmd.command) {
      ...
      case 'seek':
        ...
        break;
      case 'volume':
        if (cmd.payload?.volume !== undefined) playerRef.current?.setVolume(cmd.payload.volume);
        break;
    }
  }, []);
  ...
  // Auto-iniciar reproducción si la TV estaba en espera y entra una canción a la fila
  useEffect(() => {
    if (!isLoading && !currentSong && nextSongs.length > 0) {
  ...
  if (!currentSong) {
    return <TvIdleScreen roomCode={roomCode} joinUrl={joinUrl} roomName={room?.name || 'Fiesta Karaoke'} />;
  }
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden select-none">
      ...
      <TvFloatingQr roomCode={roomCode} joinUrl={joinUrl} />
